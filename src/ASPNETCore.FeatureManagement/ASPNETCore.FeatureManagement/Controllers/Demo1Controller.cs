...
namespace ASPNETCore.FeatureManagement.Controllers
{
    ...
    {
        public Demo1Controller(IFeatureManager featureManager)
        {
            var state = featureManager.IsEnabledAsync(nameof(FeatureNames.FeatureC), this.HttpContext);
        }
        ...
        /// </summary>
        /// <returns></returns>
        [FeatureGate(nameof(FeatureNames.FeatureC))]
        [HttpGet("attrPercentage")]
        public string GetResultWithAttributeAndPercentageFeature()
        {
            // If the FeatureA is off, it can not get here.
            return $"{nameof(FeatureNames.FeatureC)} is On";
        }
